...
- Godot 只在宿主主程序集解析 C# 脚本：挂到场景上的脚本（含 `TelepathEditorPlugin`、`FrameProviderDispatcher`、具体 View）由宿主编译；`View` / `View<T>` 等库类型在 `Telepath.Godot`。库项目不引用 `Godot.SourceGenerators`；`View` 的 `_Ready` 等方法表手写在 `View.GodotMethods.cs`。
