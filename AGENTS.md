...
- `Telepath.Godot` 对着 **GodotSharp 4.4.0**（支持下限）编译，且 `PrivateAssets=all` + `ExcludeAssets=runtime`：不要把 `GodotSharp` 传给宿主或打进 nupkg。宿主用自己的 `Godot.NET.Sdk`（Showcase 钉当前编辑器版本）。不要把库的 `GodotSharp` 升到当前引擎版本
