...
编辑器扩展源码在 `src/Telepath.Godot/Editor/`，**排除出** `Telepath.Godot` 编译，经符号链接到 `samples/Showcase/addons/Telepath/Editor`，由 Showcase 宿主编译。`FrameProviderDispatcher` 仍是 addon 根上的运行时 Autoload。
...
- Godot 只在宿主主程序集解析 C# 脚本：挂到场景上的脚本（含 `TelepathEditorPlugin`、`FrameProviderDispatcher`、具体 View）由 `Telepath.Showcase` 编译，放在 `samples/Showcase/addons/Telepath/` 或示例场景旁。编辑器脚本在 `src/Telepath.Godot/Editor/`，不编进库 DLL
