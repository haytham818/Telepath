...
`FrameProviderDispatcher` is registered as an Autoload by `TelepathEditorPlugin`.

Godot resolves C# scripts only from the **host** assembly: `FrameProviderDispatcher.cs` and `Editor/TelepathEditorPlugin.cs` are compiled by the host project, not `Telepath.Godot.dll`.
